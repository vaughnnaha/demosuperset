...
| 11 | Untrusted input isolation | Issue title and body are truncated at 20,000 characters and wrapped in a fence whose markers carry a per-run random nonce, explicitly declared data-not-instructions; marker-shaped text inside the content is stripped, so an issue author cannot close the fence and append directives | SI-10 |
